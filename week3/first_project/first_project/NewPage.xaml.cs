        ...
        private async void CreateButton(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                if (isCreate == true)
                {
                    ...
                    temp.result.AddTodoItem(TitleBlock.Text, DetailBlock.Text, Date.Date, result);
                    this.Frame.Navigate(typeof(MainPage), temp.result);
                }
                else
                {
                    BitmapImage result = NewImage.Source as BitmapImage;
                    temp.result.UpdateTodoItem(temp.id, TitleBlock.Text, DetailBlock.Text, Date.Date,result);
                    this.Frame.Navigate(typeof(MainPage), temp.result);
                }
            }
        }
